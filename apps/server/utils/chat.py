...
def get_chat_session_id(user_id: UUID, account_id: UUID, is_private_chat: bool, agent_id: UUID = None, team_id: UUID = None) -> str:
    if is_private_chat:
        ...
        return f"{team_id}-{user_id}" if team_id else f"{account_id}-{user_id}"
    else:
        # Team chat
        if agent_id:
            return f"{agent_id}"

        return f"{team_id}" if team_id else f"{account_id}"
...
def get_agents_from_json(data_string: str):
    ...
    if 'json```' not in data_string or ']```' not in data_string:
        return []
    ...
    try:
        return json.loads(json_array_data)
